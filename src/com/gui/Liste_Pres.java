...
public class Liste_Pres extends Habillage{
...
private void createGUI() {
	win = new JFrame("LISTE POUR PRECISER PRODUITS");
...
public class AddListe implements ActionListener {
	...
	try {
		 ...
		 String str = "INSERT INTO CHOIX (PRES) VALUES (?)";
		 PreparedStatement ps = connect.prepareStatement(str);
		 ps.setString(1, (String) cliste.getSelectedItem());
		 ps.executeUpdate();
	     ps.close();
			
			JOptionPane.showMessageDialog(null,"La Liste a ete modifiée");
...
public class SuppListe implements ActionListener {
	public void actionPerformed(ActionEvent ev) {
		try {
			 connect= connexion.getConnection();
			 String str = "DELETE FROM CHOIX WHERE PRES = ?";
			 PreparedStatement ps = connect.prepareStatement(str);
			 ps.setString(1, (String) cliste.getSelectedItem());
			 ps.executeUpdate();
		     ps.close();
...
private void ChargeListe() {
	...
	try {
		...
		//ajout element 1 a 1
		while(rs.next()) {
			cliste.addItem(rs.getString("PRES"));
		}
